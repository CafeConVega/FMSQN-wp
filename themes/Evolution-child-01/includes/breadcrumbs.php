<div id="breadcrumbs">
	<?php if(function_exists('bcn_display')) { bcn_display(); }
				...
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e('Home','Evolution') ?></a> <span class="raquo">&raquo;</span>
				...
				<?php } elseif (is_single()) { ?>
					<?php $category = get_the_category();
						  if ( $category ) {
							...
							// Pedro, 2015-03-18: Joshua this is a temporary hack, you need to not hard code this like I've done.
							if (esc_html($category[0]->cat_name)== "Board of Directors")
								$catlink = "http://www.fmsqn.org/?page_id=46";
							else if (esc_html($category[0]->cat_name)== "Corporate Officers")
								 $catlink = "http://www.fmsqn.org/?page_id=396";
							else if (esc_html($category[0]->cat_name)== "Network Members")
								 $catlink = "http://www.fmsqn.org/?page_id=25";
							// ------------------------------ End of new code ------------------------------
							echo ('<a href="'.esc_url($catlink).'">'.esc_html($category[0]->cat_name).'</a> '.'<span class="raquo">&raquo;</span> ');

							// Pedro, 2015-03-18: Disable the link
							//echo (esc_html($category[0]->cat_name).'<span class="raquo">&raquo;</span> ');
